...
#         # Text log
#         logger.add(log_path, rotation="30 days", retention="90 days", enqueue=True)
#         # JSON log for structured sinks
#         logger.add(
#             os.path.join("logs", self.log_name + ".json"),
...
#     def start(self):
#         """
#         Start HTTP server and ZMQ broker with worker pool.
#         """
#         # HTTP metrics & health
#         threading.Thread(target=self._run_http_server, daemon=True).start()
#         logger.info(f"Started HTTP metrics on port {self.http_port}")

#         # Setup ROUTER-DEALER broker
#         url_client = f"tcp://*:{self.rep_port}"
#         url_worker = "inproc://workers"

#         clients = self.ctx.socket(zmq.ROUTER)
#         clients.bind(url_client)
#         logger.info(f"ZMQ ROUTER bound to {url_client}")

#         workers = self.ctx.socket(zmq.DEALER)
#         workers.bind(url_worker)
#         logger.info("ZMQ DEALER bound to inproc://workers")
...
#         # Start the proxy and catch critical failures only at top level
#         try:
#             logger.info("Starting broker proxy... press Ctrl+C to exit")
#             zmq.proxy(clients, workers)
#         except Exception as e:
#             self._on_critical_error(f"Broker proxy failure: {e}")
#         finally:
...
#                     inner_self.send_response(404)
#                     inner_self.end_headers()
#                     logger.warning(f"Unknown endpoint: {inner_self.path}")
...
#     def _on_critical_error(self, err: str):
#         logger.error(f"Critical failure: {err}")
#         with self._metrics_lock:
#             self.metrics["status"] = 0
#         time.sleep(3)  # Allow log to flush
#         sys.exit(1)